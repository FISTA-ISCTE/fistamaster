        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="mt-4">
                <select name="id_curso" class="form-control mb-1" id="id_curso" required>
                    ...
                    @foreach (App\Models\Curso::all() as $curso)
                        <option value="{{ $curso->id }}" @if(old('id_curso') === $curso->id) selected @endif>{{ $curso->designacao }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mt-4">
                <select name="id_ano" class="form-control mb-1" id="id_ano" required>
                    ...
                    @foreach (App\Models\Ano::all() as $ano)
                        <option value="{{ $ano->id }}" @if(old('id_ano') === $ano->id) selected @endif>{{ $ano->designacao }}</option>
                    @endforeach
                </select>
            </div>
